...
KV.createBeveledTexture = function(cacheKey, color, size = 64, bevelSize = 3, lightColor = 'rgba(255,255,255,0.3)', darkColor = 'rgba(0,0,0,0.2)') {
...
KV.loadTexture = function(path) { // Path example: 'assets/images/textures/grass_top.png'
    if (KV.textureCache[path]) { return KV.textureCache[path]; }
    const texture = KV.textureLoader.load(path, 
        (tex) => { tex.colorSpace = THREE.SRGBColorSpace; }, 
        undefined, 
        (err) => { console.error(`Failed to load texture: ${path}`, err); }
    );
    texture.magFilter = THREE.NearestFilter; texture.minFilter = THREE.NearestFilter;
    KV.textureCache[path] = texture; return texture;
};
...
KV.textures = {
    ...
    moon_rock_surface: KV.createBeveledTexture('gen_moon_rock_surface', '#A0A0B0', 64, 2,'rgba(200,200,220,0.2)','rgba(50,50,60,0.2)'),
    moon_rock_deep: KV.createBeveledTexture('gen_moon_rock_deep', '#707080', 64, 2),
    metal_panel_light: KV.createBeveledTexture('gen_metal_panel_light', '#B0B5B8', 64, 1,'rgba(255,255,255,0.3)','rgba(0,0,0,0.2)'),
    metal_panel_dark: KV.createBeveledTexture('gen_metal_panel_dark', '#656A6E', 64, 1),
    alien_green_block: KV.createBeveledTexture('gen_alien_green', '#4CAF50', 64, 3,'rgba(120,255,120,0.3)','rgba(0,80,0,0.3)')
};

console.log("KiddoVerse Themes: Utils loaded with space textures.");

KV.getIntersection = function(camera, targetObjects, distanceLimit = Infinity) {
    const raycaster = new THREE.Raycaster(); const pointer = new THREE.Vector2(0, 0);
    raycaster.setFromCamera(pointer, camera); raycaster.far = distanceLimit;
    const intersects = raycaster.intersectObjects(targetObjects, true);
    if (intersects.length > 0) { return intersects[0]; } return null;
};
